...
`npm run test` はサーバー不要のユニットテストです（21項目）。日本語検索の中核
...
EVERNOTION_PASSWORD='設定したパスワード' npm run e2e   -- https://<あなたのアプリ>.up.railway.app
```

認証が効いているか、日本語検索・PDF抽出・OCR・エディタ・スマホ表示が
本番環境で動くかまで確認します。どちらもパスワードを渡せば自動でログインします。
