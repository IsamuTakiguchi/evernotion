...
**APIキー無しでも他の機能が壊れないこと**を検証します（66項目、
パスワードを設定した場合は認証まわりを加えて76項目）。
...
書き込み可否の実測、生成パスワードの形式、そして**すべてのAPIルートが自分でセッションを
確認していること**です。最後のものはソースを走査する形なので、
認証を忘れたルートが増えた瞬間にサーバー無しで落ちます。
...
デプロイに必要なものはリポジトリに入っています。**手で行う必要があるのは4つ**だけです
（5つめは任意）。パスワードの設定は不要になりました。
...
| 4 | **公開ドメインを生成する** | 1クリック |
| 5 | （任意）`ANTHROPIC_API_KEY` を設定する | AI機能を使う場合のみ |
...
| 変数 | 必要性 | 説明 |
|---|---|---|
| `EVERNOTION_PASSWORD` | 任意 | ログインパスワード。**未設定でも公開環境では自動生成されます**（下記） |
| `ANTHROPIC_API_KEY` | 任意 | AIチャット・自動タグ・要約用。未設定でも他は動きます |
| `EVERNOTION_SECRET` | 任意 | Cookie署名鍵。既定ではパスワードを使います |

`PORT` は Railway が自動で渡すので設定不要です。

**パスワードの自動生成**: `EVERNOTION_PASSWORD` を設定せずに公開環境（`RAILWAY_PROJECT_ID`
などが存在する環境）へデプロイすると、初回起動時に強いパスワードを生成し、
**デプロイログに1回だけ表示**してボリューム上に保存します。

```
================================================================
  Evernotion: no EVERNOTION_PASSWORD was set, so one was generated.

      jRvH7M-yQure2-dBepbo-xxtt85
...
再起動しても同じパスワードのままです（ボリュームに残るため）。
自分で決めたい場合は `EVERNOTION_PASSWORD` を設定すれば、以後そちらが優先されます。

設定を忘れても素通りにはならない、という設計です。ローカル実行では自分しかいないので、
ログイン画面は出ません。
...
```bash
EVERNOTION_PASSWORD='ログに出たパスワード' npm run smoke -- https://<あなたのアプリ>.up.railway.app
EVERNOTION_PASSWORD='ログに出たパスワード' npm run e2e   -- https://<あなたのアプリ>.up.railway.app
```
...
- **公開URLに置くとき**、`EVERNOTION_PASSWORD` を設定しなければ起動時にパスワードが
  自動生成され、デプロイログに1回だけ表示されます。ログを読み落とすと入れなくなるので、
  自分で決めておくほうが確実です。ローカル実行では認証はかかりません
